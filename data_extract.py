...
def load_data_from_structured_folders(dataset_root_path, window_size):
...
def create_sequences(data_list, label_list, window_second, step_second):
    seq_length = int(TARGET_SAMPLING_RATE_HZ * window_second)
    step = int(TARGET_SAMPLING_RATE_HZ * step_second)
    """使用滑动窗口从试验数据创建序列。"""
    # 初始化用于存放最终序列和对应标签的列表
    X, y = [], []
    # 遍历每一次活动试验的数据
...
def extract_data(window_sencond, step_second, dataseq_path, labelseq_path):
    ...
    SensorDataSequences, SensorLabelSequences = create_sequences(trial_arrays, trial_labels, window_sencond, step_second)
...
if __name__ == "__main__":
    extract_data(window_sencond=6, step_second=2,
                 dataseq_path='SensorDataSequences_reg.npy',
                 labelseq_path='SensorLabelSequences_reg.npy')
    extract_data(window_sencond=4, step_second=1,
                 dataseq_path='SensorDataSequences.npy',
                 labelseq_path='SensorLabelSequences.npy')